...
const livePreviewRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/start', async (req: any, res: any) => {
    ...
    const { url, authCookie, useProxy, pageLanguage, fingerprintProfile } = req.body as {
      url?: string;
      authCookie?: string;
      useProxy?: boolean;
      pageLanguage?: string;
    ...
    try {
      ...
      if (fingerprintProfile?.trim()) startParams.fingerprintProfile = fingerprintProfile.trim();
